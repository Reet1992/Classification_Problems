...
df4 <- read.csv(file = 'employee_att.csv')
head(df4)
...
keeps <- c("MaritalStatus","OverTime","Attrition")
df3 = df[keeps]
...
tree <- ctree(Attrition~MaritalStatus+OverTime,data = train2, control = ctree_control(mincriterion = 0.999,minsplit = 50000))
tree
plot(tree) 
...
mylogit <- glm(Attrition~MaritalStatus+OverTime, data = df, family = "binomial")
